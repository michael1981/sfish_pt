...
"This script provides the username/password for the common servers :
...
"n/a" );
 ...
 script_summary(english:"Logins for HTTP, FTP, NNTP, POP2, POP3, IMAP and SMB");
script_end_attributes();

 script_category(ACT_SETTINGS);
 script_copyright(english:"This script is Copyright (C) 2002-2009 Georges Dagousset");
...
smb_password_type = script_get_preference("SMB password type :");
...
set_kb_item(name:"SMB/password_type", value:p_type);
...
for ( i = 1 ; i <= MAX_ADDITIONAL_SMB_LOGINS ; i ++ )
{
 ...
 if ( l && p ) 
 {
   set_kb_item(name:"SMB/login_filled/" + j, value:l);
   set_kb_item(name:"SMB/password_filled/" + j, value:p);
   set_kb_item(name:"SMB/domain_filled/" + j, value:d);
   j ++;
 }
}
