
#
# (C) Tenable Network Security
#
# The text description of this plugin is (C) Novell, Inc.
#

include("compat.inc");

if ( ! defined_func("bn_random") ) exit(0);

if(description)
{
 script_id(28287);
 script_version ("$Revision: 1.4 $");
 script_name(english: "SuSE Security Update:  tomboy security update (tomboy-4698)");
 script_set_attribute(attribute: "synopsis", value: 
"The remote SuSE system is missing the security patch tomboy-4698");
 script_set_attribute(attribute: "description", value: "The tomboy mediaplayer used LD_LIBRARY_PATH unsafely by
allowing empty LD_LIBRARY_PATH components.

This would enable the player to load its shared libraries
of the current directory which might contain user supplied
shared libraries, potentially supplied from the network by
an attacker. (CVE-2005-4790)
");
 script_set_attribute(attribute: "cvss_vector", value: "CVSS2#AV:L/AC:M/Au:N/C:C/I:C/A:C");
script_set_attribute(attribute: "solution", value: "Install the security patch tomboy-4698");
script_end_attributes();

script_cve_id("CVE-2005-4790");
script_summary(english: "Check for the tomboy-4698 package");
 
 script_category(ACT_GATHER_INFO);
 
 script_copyright(english:"This script is Copyright (C) 2009 Tenable Network Security");
 script_family(english: "SuSE Local Security Checks");
 script_dependencies("ssh_get_info.nasl");
 script_require_keys("Host/SuSE/rpm-list");
 exit(0);
}

include("rpm.inc");

if ( ! get_kb_item("Host/SuSE/rpm-list") ) exit(1, "Could not gather the list of packages");
if ( rpm_check( reference:"tomboy-0.8.0-9.2", release:"SUSE10.3") )
{
	security_warning(port:0, extra:rpm_report_get());
	exit(0);
}
exit(0,"Host is not affected");
