...
if (description)
{
  ...
  script_xref(name:"OSVDB",value:"50599");
  ...
  script_set_attribute(
    ...
    value:string(
      "3CX Phone System for Windows, a software-based IP PBX, is installed\n",
      "on the remote host.  The installed version fails to sanitize input\n",
      "to the 'fName' and 'fPassword' parameters in 'login.php' before using\n",
      "it to generate an HTML response dynamically.  An unauthenticated\n",
      "remote attacker may be able to leverage these issues to inject\n",
      ...
      "likely to be affected by several other vulnerabilities, including\n",
      "denial of service, sniffing of administrator's session ID, and path\n",
      "disclosure."
    )
  );
  script_set_attribute(
    attribute:"see_also", 
    value:"http://archives.neohapsis.com/archives/fulldisclosure/2008-12/0177.html"
  );
  script_set_attribute(
    attribute:"see_also", 
    value:"http://wiki.3cx.com/change-log/build-history-changelog"
  );
  script_set_attribute(
    attribute:"solution", 
    value:"Upgrade to 3CX Phone System for Windows 7.0.3775 (RC) or later."
  );
  script_set_attribute(
