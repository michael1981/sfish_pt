...
if(description)
{
 ...
 script_xref(name:"OSVDB", value:"67");
   ...
   egrep(pattern:".*x-vermeer-rpc*", string: r[1]))
{
  ...
  {	
   ...
   myreport = 'The remote frontpage server leaks information regarding the name anonymous user\r\n';
...
if (thorough_tests)
{
 ...
 foreach file (files)
 {
   if (get_port_state(port) && is_cgi_installed3(item: file, port:port))
   {
     name = strcat('www/no404/', port);
     no404 = get_kb_item(name);
     r = http_send_recv3(item: file, method: 'POST', port:port);
     if (isnull(r)) exit(0);
