...
if(description)
{
 ...
 script_xref(name:"OSVDB", value:"128");
...
host, subject to the permissions of the web server user id." );
 script_set_attribute(attribute:"see_also", value:"http://www.cert.org/advisories/CA-1997-07.html" );
...
foreach dir (cgi_dirs())
{
  ...
  exploit = string('GET ', url, '?* HTTP/1.0');
  req = string(exploit, '\r\n\r\n');
  ...
  if (lines && !info && thorough_tests)
  {
    ...
    exploit = string('GET ', url, '?x HTTP/1.0 *');
    req = string(exploit, '\r\n\r\n');
    res = http_send_recv(port:port, data:req);
    if (res == NULL) exit(0);
