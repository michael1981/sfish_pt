
#
# (C) Tenable Network Security
#
# The text description of this plugin is (C) Novell, Inc.
#

include("compat.inc");

if ( ! defined_func("bn_random") ) exit(0);

if(description)
{
 script_id(29879);
 script_version ("$Revision: 1.2 $");
 script_name(english: "SuSE Security Update:  apache2-mod_python: Fix possible memory leak. (apache2-mod_python-4448)");
 script_set_attribute(attribute: "synopsis", value: 
"The remote SuSE system is missing the security patch apache2-mod_python-4448");
 script_set_attribute(attribute: "description", value: "This update fixes a buffer overflow in apache2-mod_python
that occurs while using python-based output-filter. This
bug can be triggered remotely to read possibly confidential
data from the process space of the web-server and in rare
cases to execute arbitrary code. (CVE-2004-2680)
");
 script_set_attribute(attribute: "cvss_vector", value: "CVSS2#AV:N/AC:L/Au:N/C:P/I:N/A:N");
script_set_attribute(attribute: "solution", value: "Install the security patch apache2-mod_python-4448");
script_end_attributes();

script_cve_id("CVE-2004-2680");
script_summary(english: "Check for the apache2-mod_python-4448 package");
 
 script_category(ACT_GATHER_INFO);
 
 script_copyright(english:"This script is Copyright (C) 2009 Tenable Network Security");
 script_family(english: "SuSE Local Security Checks");
 script_dependencies("ssh_get_info.nasl");
 script_require_keys("Host/SuSE/rpm-list");
 exit(0);
}

include("rpm.inc");

if ( ! get_kb_item("Host/SuSE/rpm-list") ) exit(1, "Could not gather the list of packages");
if ( rpm_check( reference:"apache2-mod_python-3.1.3-60.9", release:"SUSE10.1") )
{
	security_warning(port:0, extra:rpm_report_get());
	exit(0);
}
exit(0,"Host is not affected");
