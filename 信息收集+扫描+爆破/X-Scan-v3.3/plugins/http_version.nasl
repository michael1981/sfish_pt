...
remote web server." );
 ...
 script_copyright(english:"This script is Copyright (C) 2000-2009 H. Scholz & Contributors");
 script_family(english: "Web Servers");

 script_dependencie("find_service1.nasl", "http_login.nasl", "httpver.nasl", "no404.nasl", "www_fingerprinting_hmap.nasl", "webmin.nasl", "embedded_web_server_detect.nasl", "fake_http_server.nasl", "broken_web_server.nasl");
...
if("Apache" >< svr)
{
	...
	if( svr2 != NULL )
	{
	 report = report + svr2 + string("\n\nThe product version is hidden but we could determine it by\n",
	 				"requesting a nonexistent .nsf file or connecting to port 25\n");
	 svrline = string("Server: ", svr2, "\r\n");
	 if ( defined_func("replace_kb_item") )
	  	replace_kb_item(name:string("www/real_banner/", port), value:svrline);
	 else
    ...
    if(egrep(pattern:"^Server:.*(Apache.* Tomcat|Apache-Coyote)/", string:svrline, icase:1))
    ...
    if(egrep(pattern:"^Server:.*lighttpd/.*", string:svrline))
	...
	set_kb_item(name:"www/OracleApache", value:TRUE);
     ...
     if(egrep(pattern:"^Server:.*BCReport", string:svrline))
        set_kb_item(name:"www/BCReport", value:TRUE);

     if (egrep(pattern:"^Server: *eMule", string:svrline))
