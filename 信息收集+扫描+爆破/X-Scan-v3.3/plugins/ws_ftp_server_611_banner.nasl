...
multiple vulnerabilities :
  ...
  - There is a a buffer overflow vulnerability in the SSH 
...
if(!port) port = 21;
if (!get_port_state(port)) exit(1, "Couldn't connect to FTP port");

banner = get_ftp_banner(port:port);
if (!banner || "WS_FTP Server" >!< banner) exit(1, "Banner doesn't look like WS_FTP.");
