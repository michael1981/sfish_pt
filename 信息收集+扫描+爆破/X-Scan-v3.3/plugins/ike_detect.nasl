...
if(description) {
 ...
 script_name(english:"IPSEC Internet Key Exchange (IKE) Detection");
...
to this port." );
 ...
 script_summary(english:"IPSEC IKE detect");
...
function calc_data() {
    local_var blap, ISAKMP_HEADER, LEN, len1, len2, len3, len4, MU, myplen, myp_plen, tmp;
...
for (e=0; ext[e]; e++)
{
    	...
    	if (live) 
	{
		#live2 = get_udp_element(udp:live, element:"data");
        	...
        	if (  strlen(live) > 46 && (ord(live[44]) == 0x0B) && (ord(live[46]) == 0x05)  )
        	{
