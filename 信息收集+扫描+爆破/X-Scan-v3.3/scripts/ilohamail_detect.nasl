...
foreach dir (dirs) {
  ...
  foreach src (make_list("", "/source")) {
    ...
    if (!http_40x(port:port, code:res)) {
      ...
      if (
        egrep(string:res, pattern:'>Powered by <a href="http://ilohamail.org">IlohaMail<', icase:TRUE) ||
        egrep(string:res, pattern:"<h2>Welcome to IlohaMail", icase:TRUE) ||
        (
          egrep(string:res, pattern:'<input type="hidden" name="logout" value=0>', icase:TRUE) &&
          egrep(string:res, pattern:'<input type="hidden" name="rootdir"', icase:TRUE) &&
          egrep(string:res, pattern:'<input type="password" name="password" value="" size=15', icase:TRUE)
        )
      ) {
        debug_print("res =>>", res, "<<");
