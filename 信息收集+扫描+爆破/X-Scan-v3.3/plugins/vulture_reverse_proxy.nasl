...
b = http_get_cache(port: port, item: "/");
if (!isnull(b) && "vulture_app" >!< b) exit(0);

r = http_send_recv3(port: port, method: "GET", item: "/", follow_redirect:1);
if (isnull(r)) exit(0);
...
r = http_send_recv3(port: port, method: "GET", item: l[1]);
if (isnull(r)) exit(0);
