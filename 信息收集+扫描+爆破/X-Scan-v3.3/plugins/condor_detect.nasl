...
for distributed job scheduling." );
 ...
 script_set_attribute(attribute:"see_also", value:"http://en.wikipedia.org/wiki/Condor_High-Throughput_Computing_System" );
      ...
      foreach classad (split(substr(res, 17), sep:mkdword(1)+mkdword(0), keep:FALSE))
      {
        ...
        if (type == 'Scheduler' || type == 'DaemonMaster' || type = 'Negotiator')
        {
          ...
          if (addr)
          {
            ...
            alt_port = item[1];
            if (!known_service(port:alt_port) && get_tcp_port_state(alt_port))
            {
