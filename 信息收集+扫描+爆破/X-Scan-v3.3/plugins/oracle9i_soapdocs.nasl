...
if(description)
{
 ...
 script_xref(name:"OSVDB", value:"44600");
...
documentation directory, which contain the install scripts used with
the default SOAP install.  These files might be useful for an attacker
to determine which application server is in use as well as the name of
the disk where Oracle is installed. 
