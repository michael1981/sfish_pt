...
"A Windows file explorer utility is installed on the remote system." );
...
if(isnull(installstring)) exit(1, "No evidence of Total Commander is found in the Uninstaller's registry hive.");
...
if (!get_port_state(port)) exit(1,"Can't open port " + port + ".");
...
if(!isnull(ver))
{
  ...
  if(!isnull(prod_ver))
  {
   ...
   report = string(
    "\n",
    "Nessus discovered the following installation of Total Commander :\n",
    "\n",
    "  Path    : ", path, "\n",
    "  Version : ", version_ui, "\n"
  );
