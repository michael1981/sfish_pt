...
# CVE IDs ("CVE-2000-1081", "CVE-2000-0202", "CVE-2000-0485",
...
# script_bugtraq_id(13564);
 script_xref(name:"IAVA", value:"2002-t-0001");
 script_xref(name:"IAVA", value:"2002-B-0004");

 script_name(english:"Microsoft SQL Server Version Query (credentialed check)");
 
 script_set_attribute(attribute:"synopsis", value:
"The remote host is running a database application." );
...
and of the SQL server files if available to determine the Version of
SQL and Service Pack the host is running." );
 script_set_attribute(attribute:"see_also", value:"http://www.microsoft.com/sqlserver/2008/en/us/default.aspx" );
...
if ( ! isnull(key_h) )
{
 ...
 {
   sql_path = value[1];
   spath =  ereg_replace(pattern:"^(.*\\)[A-Za-z]+\.(DLL|dll).*", replace:"\1", string:sql_path);
 }
...
if (isnull(spath))
{
 ...
 if ( ! isnull(key_h) )
 {
  ...
  {
   value = RegQueryValue(handle:key_h, item:"MSSQLSERVER");
   if (!isnull (value))
     inst = value[1];
  }
 ...
 if (inst)
 {
  key = "SOFTWARE\Microsoft\Microsoft SQL Server\" + inst + "\Setup";

  key_h = RegOpenKey(handle:hklm, key:key, mode:MAXIMUM_ALLOWED);
  if ( ! isnull(key_h) )
  {
   value = RegQueryValue(handle:key_h, item:"SQLBinRoot");
   if (!isnull (value))
     spath = value[1];

   value = RegQueryValue(handle:key_h, item:"Edition");
   if (!isnull (value))
     sedition = value[1];
...
if (spath) set_kb_item(name:"mssql/path", value:spath);
if (sedition) set_kb_item(name:"mssql/edition", value:sedition);
...
share = ereg_replace(pattern:"^([A-Za-z]):.*", replace:"\1$", string:sql_path);
exe =  ereg_replace(pattern:"^[A-Za-z]:(.*)$", replace:"\1", string:sql_path);
...
exe =  ereg_replace(pattern:"(^.*\\)[A-Za-z]+\.(dll|DLL).*", replace:"\1sqlservr.exe", string:exe);
...
if( ! isnull(handle) )
{
 version3 = GetFileVersion(handle:handle);
 CloseFile(handle:handle);
}

NetUseDel();

if ((!isnull(version2)) && (ord(version[2]) < version2[2]))
  version[2] = string(version2[2]);

if ((!isnull(version3)) && (ord(version[2]) < version3[2]))
  version[2] = string(version3[2]);

if (isnull(version2) || isnull(version3))
  exit (0);
...
if (version [0] <= 7)
  report = string ("\nThe remote server is running MSSQL ", v, " (", version7[v], ").\n");
else
  report = string ("\nThe remote server is running MSSQL ", v, " (", version8[v], ").\n");
