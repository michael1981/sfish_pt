...
"The remote web server is controlling a camera. 

Anybody can view images, move the camera or change its parameters." );
 script_set_attribute(attribute:"solution", value:
"Protect it with a password or a firewall if this is not a public
webcam." );
...
for (i = 0; i < n; i ++)
{
 ...
 if (strlen(banner) > 0 && ! isnull(server[i]) && server[i] == "") continue;
 if (strlen(banner) > 0)
 {
