...
if(description)
{
 ...
 script_xref(name:"OSVDB", value:"523");
 script_xref(name:"OSVDB", value:"11911");
...
servers (including operating system and hard disk partitioning), logs
of access to users (which could expose sensitive data if GET html
forms are used).");
...
"Verify all the ACLs for the available databases and remove those that
are not needed.");
...
if(get_port_state(port))
{
 ...
 if(report)
  ...
  if(auth)
  {
   report = string(
     "\n",
     "The following databases exist but are password-protected :\n",
     "\n",
     auth
   );
