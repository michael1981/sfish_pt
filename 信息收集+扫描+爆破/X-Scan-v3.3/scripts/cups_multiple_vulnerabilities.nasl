...
if (description) {
  ...
  script_cve_id("CAN-2004-1267","CAN-2004-1268","CAN-2004-1269","CAN-2004-1270");
  script_bugtraq_id(11968,12004,12005,12007, 12200);
  ...
  name["english"] = "CUPS Multiple Vulnerabilities";
  script_name(english:name["english"]);

  desc["english"] = "
The target is running a CUPS server whose version number is between
1.0.4 and 1.1.22 inclusive.  Such versions are prone to multiple
vulnerabilities :

- A remotely exploitable buffer overflow in the hpgltops filter that
enable specially crafted HPGL files can execute arbitrary commands as
the CUPS 'lp' account;

- A local user may be able to prevent anyone from changing his or
her password until a temporary copy of the new password file is
cleaned up (lppasswd flaw);

- A local user may be able to add arbitrary content to the password
file by closing the stderr file descriptor while running lppasswd
(lppasswd flaw);

- A local attacker may be able to truncate the CUPS password file,
thereby denying service to valid clients using digest authentication.
(lppasswd flaw).
