...
function decode_entry (entry)
{
 ...
 {
  ret[1] = string ("Named pipe : ", get_string (blob:part2, pos:0, _type:0));
  return ret;
 }
 ...
 if (protocol == 0x11)
 {
  ret[1] = string ("Netbios name : ", get_string (blob:part2, pos:0, _type:0));
  return ret;
 }
 ...
 if (!isnull(values))
 {
  ...
  for (i=1; i<max_index(values); i++)
  {
   ...
   if (!isnull(ret))
   {
	...
	{
          register_service(port:entry4[2], proto:string("DCE/", entry1[2]), ipproto: "udp");
