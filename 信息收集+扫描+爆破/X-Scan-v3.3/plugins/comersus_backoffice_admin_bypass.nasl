...
if (description) {
  ...
  script_xref(name:"OSVDB", value:"49528");

  script_name(english:"Comersus BackOffice comersus_backoffice_menu.asp Multiple Variable SQL Injection");
...
application and its underlying database." );
 script_set_attribute(attribute:"see_also", value:"http://web.archive.org/web/20060810181145/http://www.morx.org/comersus.txt" );
...
foreach dir (dirs) {
  foreach prod (make_list("backofficeLite", "backofficePlus")) {
    ...
    if (egrep(string: "Set-Cookie.*:.*ASPSESSIONID", pattern: r[1]))
    {
