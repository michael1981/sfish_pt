...
if (!thorough_tests) exit(0, "Thorough tests is not set");
...
if (!get_tcp_port_state(port)) exit(0, "Port is closed");


soc = open_sock_tcp(port);
if (!soc) exit(0, "Connection refused");
...
if (strlen(res1_1) != 4) exit(0, "Data does match expected value");
len = getdword(blob:res1_1, pos:0);
if (len == 0) exit(0, "Length is null");
if (len > 1024*1024) exit(0, "Length is too big");
res1_2 = recv(socket:soc, length:len);
if (strlen(res1_2) != len) exit(0, "Short read");
res1 = res1_1 + res1_2;
...
)
{
  ...
  if (strlen(res2_1) != 4) exit(0, "Short read");
  len = getdword(blob:res2_1, pos:0);
  if (len == 0) exit(0, "Length is null");
  if (len > 1024*1024) exit(0, "Length is too big");
  res2_2 = recv(socket:soc, length:len);
  if (strlen(res2_2) != len) exit(0, "Short read");
  res2 = res2_1 + res2_2;
