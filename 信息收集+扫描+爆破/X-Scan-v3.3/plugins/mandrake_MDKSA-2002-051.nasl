
#
# (C) Tenable Network Security
#
# This plugin text was extracted from Mandrake Linux Security Advisory ADVISORY
#

include("compat.inc");

if ( ! defined_func("bn_random") ) exit(0);
if(description)
{
 script_id(13954);
 script_version ("$Revision: 1.7 $");
 script_name(english: "MDKSA-2002:051: xchat");
 script_set_attribute(attribute: "synopsis", value: 
"The remote host is missing the patch for the advisory MDKSA-2002:051 (xchat).");
 script_set_attribute(attribute: "description", value: "In versions of the xchat IRC client prior to version 1.8.9, xchat does
not filter the response from an IRC server when a /dns query is
executed. xchat resolves hostnames by passing the configured resolver
and hostname to a shell, so an IRC server may return a malicious
response formatted so that arbitrary commands are executed with the
privilege of the user running xchat.
");
 script_set_attribute(attribute: "cvss_vector", value: "CVSS2#AV:N/AC:L/Au:N/C:P/I:P/A:P");
script_set_attribute(attribute: "see_also", value: "http://wwwnew.mandriva.com/security/advisories?name=MDKSA-2002:051");
script_set_attribute(attribute: "solution", value: "Apply the newest security patches from Mandriva.");
script_end_attributes();

script_cve_id("CVE-2002-0382");
script_summary(english: "Check for the version of the xchat package");
 
 script_category(ACT_GATHER_INFO);
 
 script_copyright(english:"This script is Copyright (C) 2009 Tenable Network Security");
 script_family(english: "Mandriva Local Security Checks");
 script_dependencies("ssh_get_info.nasl");
 script_require_keys("Host/Mandrake/rpm-list");
 exit(0);
}

include("rpm.inc");

if ( ! get_kb_item("Host/Mandrake/rpm-list") ) exit(1, "Could not get the list of packages");

if ( rpm_check( reference:"xchat-1.8.9-1.2mdk", release:"MDK7.1", yank:"mdk") )
{
 security_hole(port:0, extra:rpm_report_get());
 exit(0);
}
if ( rpm_check( reference:"xchat-1.8.9-1.2mdk", release:"MDK7.2", yank:"mdk") )
{
 security_hole(port:0, extra:rpm_report_get());
 exit(0);
}
if ( rpm_check( reference:"xchat-1.8.9-1.1mdk", release:"MDK8.0", yank:"mdk") )
{
 security_hole(port:0, extra:rpm_report_get());
 exit(0);
}
if ( rpm_check( reference:"xchat-1.8.9-1.1mdk", release:"MDK8.1", yank:"mdk") )
{
 security_hole(port:0, extra:rpm_report_get());
 exit(0);
}
if ( rpm_check( reference:"xchat-1.8.9-1.1mdk", release:"MDK8.2", yank:"mdk") )
{
 security_hole(port:0, extra:rpm_report_get());
 exit(0);
}
if (rpm_exists(rpm:"xchat-", release:"MDK7.1")
 || rpm_exists(rpm:"xchat-", release:"MDK7.2")
 || rpm_exists(rpm:"xchat-", release:"MDK8.0")
 || rpm_exists(rpm:"xchat-", release:"MDK8.1")
 || rpm_exists(rpm:"xchat-", release:"MDK8.2") )
{
 set_kb_item(name:"CVE-2002-0382", value:TRUE);
}
exit(0, "Host is not affected");
