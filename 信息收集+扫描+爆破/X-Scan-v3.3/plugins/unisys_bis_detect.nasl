...
information access tool, is listening on the remote host." );
  ...
  script_set_attribute(attribute:"see_also", value:"http://www.nessus.org/u?0d9673df (registration required)" );
...
)
{
  ...
  if (!port ) exit(1, "Service listening on port "+ port + " is not unknown.");
  if (!silent_service(port)) exit(1, "Service listening on port "+ port + " not a silent service.");
}
else port = 3986;
...
if(!res) exit(1, "The service on port "+port+" did not respond to the first request.");
