...
Risk factor : High";
 ...
 summary["english"] = "Sends an HTTP request with %s inside a HTTP header";
...
"Max-Forwards", "TE" ))
foreach bad (make_list("%08x", "%s", "%08x%x%s%p%n%d%o%u%c%h%l%q%j%z%Z%t%i%e%g%f%a%C%S%08x%%x%%s%%p%%n%%d%%o%%u%%c%%h%%l%%q%%j%%z%%Z%%t%%i%%e%%g%%f%%a%%C%%S%%08x"))
{
