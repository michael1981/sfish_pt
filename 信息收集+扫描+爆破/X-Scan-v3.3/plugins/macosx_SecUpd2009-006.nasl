...
if (description)
{
  ...
  script_xref(name:"OSVDB", value:"38484");
  script_xref(name:"OSVDB", value:"41948");
  script_xref(name:"OSVDB", value:"43306");
...
if (ereg(pattern:"^(9\.[0-8]\.)", string:darwin))
{
  ...
  if (egrep(pattern:"^com\.apple\.pkg\.update\.security\.2009\.006\.bom", string:packages))
    exit(0, "The host has Security Update 2009-006 installed and is not affected.");
  else
    security_hole(0);
}
else exit(0, "The host is running Darwin kernel version "+darwin+" and is not affected.");
